...
impl Node {
    fn calculate(location: &Point, start: &Point, end: &Point, path_to_parrent: Vec<u32>) -> Node {
        let g_cost = (((square_i32(location.x - start.x) + square_i32(location.y - start.y))
            as f32)
            .sqrt()) as u32;
        let h_cost = (((square_i32(location.x - end.x) + square_i32(location.y - end.y)) as f32)
            .sqrt()) as u32;
...
pub fn path_finder(
    ...
    loop {
        ...
        current = open.iter().min_by_key(|node| node.f_cost).unwrap().clone();

        // remove the current node from open
        open.retain(|node| !Node::compare(&current, node));

        // add open to the closed list
        closed.push(Node::calculate(
            &current.location,
            &start,
            &end,
            current.path_to_parrent.clone(),
        ));
        ...
        for (x_dir, y_dir) in directions.iter() {
            ...
            if closed
                .iter()
                .any(|node| neighbour.location == node.location)
            {
                continue;
            }
